...
<head>
	<meta charset="utf-8">
	<title>JBoss Healthcare Demo</title>
...
$( "#dialog-link, #icons li" ).hover(
	...
	$("#button").click(function(){
		$.ajax({
		   type: "GET",
		   dataType: "jsonp",
		   url: "http://healthwebconsole-demo.rhel-cdk.10.1.2.2.xip.io/his/registry/"+$('input[name=firstname]').val()+"/"+$('input[name=familyname]').val()+"/"+$('input[name=hisId]').val(),
    ...
    if($('input[name=observation]').val() == null || $('input[name=observation]').val() == ""){
		...
		$.ajax({
		   type: "GET",
		   dataType: "jsonp",
		   url: "http://healthwebconsole-demo.rhel-cdk.10.1.2.2.xip.io/his/dotest/"+$('input[name=hisId]').val()+"/"+$( "#dept option:selected" ).val()+"/"+$('input[name=testdetail]').val()+"/"+$('input[name=observation]').val(),
		   success: function(data){
		     alert(data);
		   }
	...
	$("#prescribe").click(function(){
		$.ajax({
		   type: "GET",
		   dataType: "jsonp",
		   url: "http://healthwebconsole-demo.rhel-cdk.10.1.2.2.xip.io/his/prescribe/"+$('input[name=hisId]').val()+"/"+$('#interval option:selected').val()+"/"+$('#drugs option:selected').val(),
		   success: function(data){
		     alert(data);
		   }
...
$(document).ready(function() {
  ...
  connect();
    ...
    try{
			...
			var clinicsocket = new WebSocket("ws://clinicservice-demo.rhel-cdk.10.1.2.2.xip.io/demo");
    	...
    	function clinicMessage(msg){$('#clinicDiv').html(message(clinicConsoleArray,msg));}
			...
			var labsocket = new WebSocket("ws://laboratoryservice-demo.rhel-cdk.10.1.2.2.xip.io/demo");
    	...
    	function labMessage(msg){$('#labDiv').html(message(labConsoleArray,msg));}
			...
			var radiologysocket = new WebSocket("ws://radiologyservice-demo.rhel-cdk.10.1.2.2.xip.io/demo");
    	...
    	var esbmsgsocket = new WebSocket("ws://hisesb-demo.rhel-cdk.10.1.2.2.xip.io/demo");
...
function populateTable() {
    ...
    $.get( 'http://healthwebconsole-demo.rhel-cdk.10.1.2.2.xip.io/his/allpatients', function( data ) {
...
function populatePharmacyTable(pharmacyhisid) {
    ...
    var tableContent = '';
	  var pharmacyUrl = 'http://healthwebconsole-demo.rhel-cdk.10.1.2.2.xip.io/his/pharmacy/'+pharmacyhisid;
